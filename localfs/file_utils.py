...
class FileUtils:
    ...
    def validate_file_path(file_path: str, root_dir: str) -> str:
        try:
            ...
            if not resolved_path.is_relative_to(root_path):
                raise ValueError(
                    f"Path '{file_path}' is outside the allowed root directory '{root_dir}'"
                )
    ...
    def list_directory_contents(
        directory_path: str, root_dir: str, limit: int = 100, recursive: bool = False
    ) -> str:
    ...
    def _match_path_name(item: Path, keywords: list[str], regex_keywords: bool) -> bool:
        ...
        for keyword in keywords:
            if regex_keywords:
                if re.search(keyword, item.name) or any(
                    re.search(keyword, Path(p).name) for p in item.parts if p
                ):
                    return True
            elif keyword in item.name or any(
                keyword in str(p) for p in item.parts if p
            ):
                return True
        return False
    ...
    def _match_file_content(
        item: Path, keywords: list[str], regex_keywords: bool
    ) -> bool:
        if not keywords:
            return True
        try:
    ...
    def find_files(
        path: str,
        root_dir: str,
        keywords_path_name: list[str],
        keywords_file_content: list[str],
        ...
        for item in path_obj.rglob("*"):
            if item.is_file():
                path_name_match = FileUtils._match_path_name(
                    item, keywords_path_name, regex_keywords
                )
                content_match = FileUtils._match_file_content(
                    item, keywords_file_content, regex_keywords
                )
    ...
    def _read_text_content(
        ...
        if full_content.endswith(
            ("\r\n", "\n", "\r")
        ) and not normalized_full_content.endswith("\n"):
        ...
        if offset_type == OffsetType.LINE:
            ...
            end_char = (
                len(normalized_full_content)
                if end_offset_inclusive == -1
                else end_offset_inclusive + 1
            )
            processed_content_segment = normalized_full_content[
                start_offset_inclusive:end_char
            ]
            processed_content_lines = processed_content_segment.splitlines(
                keepends=True
            )

            start_line_for_display = (
                normalized_full_content[:start_offset_inclusive].count("\n") + 1
            )
            ...
            processed_content_bytes_segment = full_content_bytes[
                start_offset_inclusive:end_byte
            ]
            processed_content_segment = processed_content_bytes_segment.decode(
                file_encoding, errors="ignore"
            )
            processed_content_lines = FileUtils.normalize_line_endings(
                processed_content_segment, "\n"
            ).splitlines(keepends=True)

            char_offset_at_start_byte = len(
                full_content_bytes[:start_offset_inclusive].decode(
                    file_encoding, errors="ignore"
                )
            )
            start_line_for_display = (
                normalized_full_content[:char_offset_at_start_byte].count("\n") + 1
            )
        if output_format == OutputFormat.RAW_UTF8:
            return "".join(processed_content_lines)
        numbered_lines = [
            f"{i + start_line_for_display: >4} | {line.rstrip('\n')}"
    ...
    def read_file_content(
        ...
        try:
            ...
            if not is_text:
                ...
                return FileUtils._encode_content(
                    FileUtils._read_binary_content(
                        path_obj,
            ...
            return FileUtils._read_text_content(
                path_obj,
                start_offset_inclusive,
                end_offset_inclusive,
                offset_type,
    ...
    @staticmethod
    def _write_text_content_and_get_message(
        path_obj: Path, file_path: str, new_content: str, file_encoding: str
    ) -> tuple[str, str | None]:
        original_content = None
        if path_obj.exists():
            original_content = path_obj.read_text(encoding=file_encoding)
        path_obj.write_text(new_content, encoding=file_encoding)
        message, diff = FileUtils.get_write_success_message_text(
            file_path, new_content, original_content
        )
        return message, diff

    @staticmethod
    def _write_binary_content_and_get_message(
        path_obj: Path, file_path: str, new_content: bytes
    ) -> str:
    ...
    def write_file_content(
        ...
        try:
            if mode == "binary":
                message = FileUtils._write_binary_content_and_get_message(
                    path_obj, file_path, new_content
                )
            else:
                message, diff = FileUtils._write_text_content_and_get_message(
                    path_obj, file_path, new_content, file_encoding
                )
                if diff:
                    message += f"\n\nDiff:\n{diff}"
    ...
    @staticmethod
    def validate_and_decode_content(
        content: str | bytes, mode: str, input_format: InputFormat
    ) -> str | bytes:
    ...
    @staticmethod
    def get_write_success_message_text(
        file_path: str, content: str, original_content: str | None
    ) -> tuple[str, str | None]:
    ...
    def modify_file(
        ...
        FileUtils.logger.info(
            f"FileUtils.modify_file received: file_path={file_path}, root_dir={root_dir}, start_offset_inclusive={start_offset_inclusive}, end_offset_inclusive={end_offset_inclusive}, offset_type={offset_type.name}, new_content_type={type(new_content)}, input_format={input_format.name}, file_encoding={file_encoding}, mode={mode}"
        )
        validated_path = FileUtils.validate_file_path(file_path, root_dir)
        path_obj = Path(validated_path)
        ...
        if mode == "text":
            original_content = path_obj.read_text(encoding=file_encoding)
            processed_new_content = FileUtils.validate_and_decode_content(
                new_content, "text", input_format
            )
            if not isinstance(processed_new_content, str):
                raise ValueError(
                    "new_content must be a string for text files after decoding."
                )
            ...
            if offset_type == OffsetType.LINE:
                ...
                modified_lines = (
                    ...
                    + processed_new_content.splitlines(keepends=True)
                    + lines[end_line:]
                )
                ...
                modified_bytes = (
                    content_bytes[:start_offset_inclusive]
                    + processed_new_content.encode(file_encoding)
                    + content_bytes[end_byte:]
                )
                modified_content = modified_bytes.decode(file_encoding)
            else:
                raise ValueError(
                    f"Offset type {offset_type.name} not supported for text files."
                )

            path_obj.write_text(modified_content, encoding=file_encoding)
            diff_output = FileUtils.generate_diff(
                original_content, modified_content, file_path
            )
            return f"Successfully modified text file '{file_path}'.\n\nDiff:\n{diff_output}"
        elif mode == "binary":
            processed_new_content_bytes = FileUtils.validate_and_decode_content(
                new_content, "binary", input_format
            )
            if not isinstance(processed_new_content_bytes, bytes):
                raise ValueError(
                    "new_content must be bytes for binary files after decoding."
                )

            original_content_bytes = path_obj.read_bytes()
            FileUtils.logger.info(
                f"modify_file (binary mode): original_content_bytes type: {type(original_content_bytes)}, value: {len(original_content_bytes)} bytes"
            )
            FileUtils.logger.info(
                f"modify_file (binary mode): start_offset_inclusive: {start_offset_inclusive}, end_offset_inclusive: {end_offset_inclusive}"
            )

            if offset_type == OffsetType.BYTE:
                end_byte = (
                    len(original_content_bytes)
                    if end_offset_inclusive == -1
                    else end_offset_inclusive + 1
                )
                modified_content_bytes = (
                    original_content_bytes[:start_offset_inclusive]
                    + processed_new_content_bytes
                    + original_content_bytes[end_byte:]
                )
                FileUtils.logger.info(
                    f"modify_file (binary mode): modified_content_bytes type: {type(modified_content_bytes)}, value: {len(modified_content_bytes)} bytes"
                )
    ...
    def _replace_content_in_binary_file(
        ...
        if is_regex:
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid regex pattern for binary content: {e}"
                ) from e
        elif number_of_occurrences == -1:
            modified_content_bytes = original_content_bytes.replace(
                old_content, new_content
            )
            replacements_made = original_content_bytes.count(old_content)
        else:
            modified_content_bytes = original_content_bytes.replace(
                old_content, new_content, number_of_occurrences
            )
            replacements_made = original_content_bytes.count(old_content)
    ...
    def replace_in_file(
        ...
        if mode == "text":
            ...
            processed_old_content = FileUtils.validate_and_decode_content(
                old_content, "text", input_format
            )
            processed_new_content = FileUtils.validate_and_decode_content(
                new_content, "text", input_format
            )

            if not isinstance(processed_old_content, str) or not isinstance(
                processed_new_content, str
            ):
                raise ValueError(
                    "old_content and new_content must be strings for text files after decoding."
                )

            original_content_normalized = FileUtils.normalize_line_endings(
                original_content_text, "\n"
            )
            old_content_normalized = FileUtils.normalize_line_endings(
                processed_old_content, "\n"
            )
            new_content_normalized = FileUtils.normalize_line_endings(
                processed_new_content, "\n"
            )
            ...
            if is_regex:
                try:
                    modified_content, replacements_made = re.subn(
                        ...
                        count=(
                            0 if number_of_occurrences == -1 else number_of_occurrences
                        ),
                    )
                except re.error as e:
                    raise ValueError(
                        f"Invalid regex pattern for text content: {e}"
                    ) from e
            elif number_of_occurrences == -1:
                modified_content = original_content_normalized.replace(
                    old_content_normalized, new_content_normalized
                )
                replacements_made = original_content_normalized.count(
                    old_content_normalized
                )
            else:
                modified_content = original_content_normalized.replace(
            ...
            diff_output = FileUtils.generate_diff(
                original_content_text, modified_content, file_path
            )
            return f"Successfully replaced {replacements_made} occurrence(s) in text file '{file_path}'.\n\nDiff:\n{diff_output}"

        elif mode == "binary":
            processed_old_content_bytes = FileUtils.validate_and_decode_content(
                old_content, "binary", input_format
            )
            processed_new_content_bytes = FileUtils.validate_and_decode_content(
                new_content, "binary", input_format
            )

            if not isinstance(processed_old_content_bytes, bytes) or not isinstance(
                processed_new_content_bytes, bytes
            ):
                raise ValueError(
                    "old_content and new_content must be bytes for binary files after decoding."
                )

            (
                modified_content_bytes,
                replacements_made,
            ) = FileUtils._replace_content_in_binary_file(
                path_obj,
                processed_old_content_bytes,
                processed_new_content_bytes,
                number_of_occurrences,
                is_regex,
            )
    ...
    def delete_paths(paths: list[str], root_dir: str) -> dict:
        ...
        return {
            "status": "success",
            "message": f"Successfully deleted {len(deleted_items)} item(s): {', '.join(deleted_items)}",
        }
