...
from orchestrator.mcp.servers.localfs.file_utils import (
    FileUtils,
    InputFormat,
    OffsetType,
    OutputFormat,
...
class LocalFiles:
    ...
    def __init__(self, root_dir: str):
        self.tools = {
            tool_decl["name"]: getattr(self, tool_decl["name"])
            for tool_decl in get_tool_declarations()
        }
        ...
        if not logging.root.handlers:
            ...
            log_file = (
                logs_dir
                / f"mcp_server_debug_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.log"
            )
            logging.basicConfig(
                filename=log_file,
                level=logging.DEBUG,
    ...
    def _send_response(self, response, stdout):
        ...
        try:
            ...
            stdout.flush()
            self.logger.debug(
                "Response sent successfully: %s. Raw response: %s",
    ...
    def _validate_file(self, file_path: str) -> tuple[bool, str]:
        ...
        if not (
            file_type_info
            and file_type_info.get("validation_command")
            and file_type_info["validation_command"] != "none"
        ):
            self.logger.info(
                f"No validation command specified or found for {file_path}."
            )
            return True, "No validation command specified or found."

        if not FileUtils.is_text_file(Path(file_path)):
            self.logger.info(
                f"Skipping text-based validation for non-text file: {file_path}"
            )
        ...
        validation_command_str = file_type_info["validation_command"].replace(
        ...
        try:
            ...
            if result.timed_out:
                self.logger.error(
                    f"Validation command '{validation_command_str}' timed out for {file_path}."
                )
                return False, "Validation command timed out after 15 seconds."

            if result.return_code == 0:
                self.logger.info(
                    f"Validation command '{validation_command_str}' succeeded for {file_path}. Output: {output or 'No issues found.'}"
                )
                return True, output or "No issues found."

            self.logger.error(
                f"Validation command '{validation_command_str}' failed for {file_path} with exit code {result.return_code}. Error: {output}"
            )
            return False, output
    ...
    def _filter_tool_arguments(self, tool_name: str, tool_args: dict) -> dict:
        ...
        if not expected_params:
            self.logger.info(
                f"_filter_tool_arguments: No expected parameters for {tool_name}. Returning original args: {tool_args}"
            )
            return tool_args

        self.logger.info(
            f"_filter_tool_arguments: Original arguments for {tool_name}: {tool_args}"
        )
    ...
    def read_from_file(
        ...
        try:
            ...
            if is_binary:
                ...
                self.logger.info("File is binary, forcing offset_type to BYTE.")
            else:
    ...
    def _get_write_success_message(
        self,
        file_path: str,
        content: str | bytes,
        mode: str,
        original_content: str | bytes | None,
    ) -> dict:
        """Generates the success message for write_file, dispatching to specific handlers."""
        if mode == "binary":
            message = FileUtils.get_write_success_message_binary(
                file_path, cast(bytes, content)
            )
            return {"message": message, "diff": None}
        message, diff_output = FileUtils.get_write_success_message_text(
            file_path, cast(str, content), cast(str | None, original_content)
        )
    ...
    def write_to_file(
        ...
        try:
            ...
            self.logger.info(
                "Writing file: %s (mode: %s, input_format: %s, file_encoding: %s)",
                path,
                mode,
            ...
            processed_content_raw = FileUtils.validate_and_decode_content(
                new_content, mode, input_format_enum
            )
            ...
            with open(
                tmp_file_path,
                "w+" if mode == "text" else "wb+",
                encoding=file_encoding if mode == "text" else None,
            ) as tmp_file:
            ...
            try:
                ...
                if not is_valid:
                    raise ValueError(
                        f"Validation failed for temporary file {tmp_file_path}. Original file {path} remains unchanged. Validator output: {validation_output}"
                    )
                ...
                result_dict = self._get_write_success_message(
                    path,
                    processed_content,
                    mode,
                    cast(str | bytes | None, original_content),
                )
    ...
    def list_dir(self, path: str, limit: int = 100, recursive: bool = False) -> str:
        ...
        try:
            self.logger.info(
                "Listing directory: %s (limit: %s, recursive: %s)",
                path,
                limit,
                recursive,
            )
    ...
    def find_paths(
        ...
        kewords_file_content: list[str] | None = None,
        regex_keywords: bool = False,
    ) -> list[str]:
        """Searches for files based on keywords in path/name or content."""
        try:
            self.logger.info(
                "Finding paths in %s (path_keywords: %s, content_keywords: %s, regex: %s)",
                path,
                keywords_path_name,
            ...
            with open(
                ...
                if original_content is not None:
                    if mode == "text":
                        ...
                        content_to_write = (
                            cast(str, original_content)
                            if isinstance(original_content, str)
                            else original_content.decode(file_encoding)
                        )
                        tmp_file.write(content_to_write)
                    else:
                        # Ensure original_content is bytes for binary mode
                        content_to_write = (
                            cast(bytes, original_content)
    ...
    def replace_in_file(
        ...
        try:
            input_format_enum = InputFormat[input_format.upper()]
            logging.info(
                "Replacing content in file %s (input_format: %s, file_encoding: %s)",
                path,
                input_format_enum.name,
                file_encoding,
            )
            ...
            if file_exists:
                try:
                    ...
                    if mode == "binary":
                        original_content = base64.b64decode(
                            read_content.encode("ascii")
                        )
                    else:
                        original_content = read_content
                except Exception as e:
                ...
                if original_content is not None:
                    if mode == "text":
                        ...
                        content_to_write = (
                            cast(bytes, original_content)
                            if isinstance(original_content, bytes)
                            else original_content.encode(file_encoding)
                        )
                        tmp_file.write(content_to_write)
        ...
        if method == "tools/call":
            ...
            try:
                ...
                if isinstance(e, (ValueError, PermissionError, FileNotFoundError)):
                    ...
                    self._send_error(
                        f"Internal server error during tool '{tool_name}' execution: {e}",
                        request_id,
                        stdout=stdout,
                    )
            return
