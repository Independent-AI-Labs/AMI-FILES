...
def setup_ami_paths() -> tuple[Path | None, Path, str]:
    ...
    while module_root.parent != module_root:
        ...
        if (
            (module_root / "backend").exists() or (module_root / "scripts").exists()
        ) and (
            (module_root / "requirements.txt").exists()
            or (module_root / ".venv").exists()
        ):
            break
    ...
    while orchestrator_root and orchestrator_root.parent != orchestrator_root:
        if (orchestrator_root / ".git").exists() and (
            orchestrator_root / "base"
        ).exists():
            break
